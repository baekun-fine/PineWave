...
class _FV3_(allpassm)
{
 public:
  ...
  inline _fv3_float_t _process(_fv3_float_t input, _fv3_float_t modulation)
  {
    ...
    // IIR low-pass smoothed fractional interpolation:
    // re-uses previous output (z_1) to reduce zipper / modulation artifacts
    // adaptive cutoff depending on m_frac; see _process_li() for plain linear.
